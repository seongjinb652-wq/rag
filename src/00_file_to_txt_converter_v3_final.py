...
class DocumentConverterV3:
    ...
    def extract_hwp_text(self, file_path):
        """HWP 파일의 깨짐을 최소화한 텍스트 추출 (v4 로직 적용)"""
        try:
            ...
            # 본문 데이터가 담긴 Section들 찾기 (BodyText/Section0, Section1...)
            ...
            f.close()
            # 한글 및 주요 문장부호 정규식 필터링
            full_text = re.sub(r'[^\w\s\.\,\?\!\(\)\[\]\%\:\-\d\uAC00-\uD7A3]+', ' ', full_text)
            return full_text
        except Exception as e:
